...
- **Resolved (2026-09-15, this commit) — the voyage log gets a reserved floor.** `compose` costs the chronological surface first and trims only tools and recent, and the last-resort cut takes index lines out of the graph instead of characters off the payload's tail; the log now leaves a payload only when it is empty or wider than the whole budget.
...
- **Resolved (2026-09-15, this commit) — the description names the trim.** `RECALL_DESCRIPTION` now says not every surface arrives, that `trimmed.sections` names the ones the budget dropped and `trimmed.banks_degraded` counts the banks it cut back, and that the voyage log's latest entry is carried whenever the log has one.
